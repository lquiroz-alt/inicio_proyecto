<?php
	...
	try{
		...
		for($i = 0; $i < 1200; $i++){
			...
			$sql_destino = "SELECT c.cuenta_id, c.numero_cuenta, c.dinero, t.nombre_cuenta FROM cuentas c
				INNER JOIN tipo_cuenta t ON t.tipo_cuenta_id = c.tipo_cuenta_id
				WHERE c.cuenta_id = :cuenta_id";

			$resultado=$base->prepare($sql_destino);
			$resultado->bindValue(":cuenta_id", $cuenta_destino);
			$resultado->execute();
			$obtener_datos_destino = $resultado->fetch(PDO::FETCH_OBJ);

			if($obtener_datos_cliente->dinero >= $deposito){
				/////Se hace el deposito
				$restar = $obtener_datos_cliente->dinero - $deposito;
				$time_start = microtime(true);
				$sql_deposita = "UPDATE cuentas SET dinero = :restar WHERE cuenta_id = :cuenta_id";
				$resultado=$base->prepare($sql_deposita);
				$resultado->bindValue(":restar", $restar);
				$resultado->bindValue(":cuenta_id", $cliente);
				$resultado->execute();

				$sumar = $obtener_datos_destino->dinero + $deposito;
				$sql_deposita = "UPDATE cuentas SET dinero = :sumar WHERE cuenta_id = :cuenta_id";
				$resultado=$base->prepare($sql_deposita);
				$resultado->bindValue(":sumar", $sumar);
				$resultado->bindValue(":cuenta_id", $cuenta_destino);
				$resultado->execute();
				...
				$resultado->bindValue(":descripcion", "Pago interno", PDO::PARAM_STR);
				...
				$resultado->bindValue(":cuenta_destino", $obtener_datos_destino->numero_cuenta, PDO::PARAM_STR);
				$resultado->bindValue(":deleted", true, PDO::PARAM_BOOL);
				$resultado->execute();

				//Historia cliente destino
				$resultado=$base->prepare($sql_inserta);
				$resultado->bindValue(":cuenta_id", $obtener_datos_destino->cuenta_id, PDO::PARAM_INT);
				$resultado->bindValue(":descripcion", "Recibo de pago", PDO::PARAM_STR);
				$resultado->bindValue(":numero_rastreo", $rastreo, PDO::PARAM_INT);
				$resultado->bindValue(":cantidad", $deposito, PDO::PARAM_STR);
				$resultado->bindValue(":sitio","Banco Fortuna", PDO::PARAM_STR);
				$resultado->bindValue(":tipo_transferencia_id", 2, PDO::PARAM_INT);
				$resultado->bindValue(":cuenta_destino", $obtener_datos_cliente->numero_cuenta, PDO::PARAM_STR);
				$resultado->bindValue(":deleted", true, PDO::PARAM_BOOL);
				$resultado->execute();
				$time_end = microtime(true);
				...
				$resultado->bindValue(":operacion", "Deposito", PDO::PARAM_STR);////Operacion que se hizo
				...
				////// No se realiza por falta de dinero
				$sql_log = "INSERT INTO log (operacion,tiempo_operacion,cuenta,tipo_transaccion,accion,respuesta) VALUES (:operacion,:tiempo_operacion,:cuenta,:tipo_transaccion,:accion,:respuesta)";
				$resultado=$base->prepare($sql_log);
				$resultado->bindValue(":operacion", "Deposito", PDO::PARAM_STR);////Operacion que se hizo
...
<body>
	<h2>Ejecutando clientes: Depositos :)</h2>
</body>
</html>